...
static int multi_seg_required(void) {
  return (MTU_TO_FRAMELEN(mtu_size) + RTE_PKTMBUF_HEADROOM > frame_buf_size ||
          frag_tbl_sz != 0);
}
...
static inline uint32_t send_fragment_packet(struct lcore_conf* qconf,
  ...
  if (proto == IPPROTO_IP)
    rc = rte_ipv4_fragment_packet(m, tbl->m_table + len, n, mtu_size,
                                  qconf->frag.pool_dir, qconf->frag.pool_indir);
  else
    rc = rte_ipv6_fragment_packet(m, tbl->m_table + len, n, mtu_size,
                                  qconf->frag.pool_dir, qconf->frag.pool_indir);
...
static inline void process_pkts_outbound(struct ipsec_ctx* ipsec_ctx,
  ...
  if (app_sa_prm.enable == 0) {
    nb_pkts_out =
        ipsec_outbound(ipsec_ctx, traffic->ipsec.pkts, traffic->ipsec.res,
                       traffic->ipsec.num, MAX_PKT_BURST);
...
/* Create a fresh mbuf containing: [ETH][IP][original-packet-bytes] */
struct rte_mbuf* prepend_eth_ip_and_replace(struct rte_mbuf* m,
                                            struct rte_mempool* pool,
                                            struct rte_ether_addr* src_mac,
                                            struct rte_ether_addr* dst_mac,
                                            uint32_t src_ip, /* CPU order */
                                            uint32_t dst_ip) /* CPU order */
{
  if (!m || !pool || !src_mac || !dst_mac)
    return NULL;

  const uint16_t hdr_len =
      sizeof(struct rte_ether_hdr) + sizeof(struct rte_ipv4_hdr);
  const uint32_t payload_len = rte_pktmbuf_pkt_len(m);
  ...
  /* Reserve header area first */
  void* hdr_ptr = rte_pktmbuf_append(new_m, hdr_len);
  if (hdr_ptr == NULL) {
  ...
  /* Reserve payload area after header */
  void* payload_ptr = rte_pktmbuf_append(new_m, payload_len);
  if (payload_ptr == NULL) {
  ...
  /* Copy payload from possibly multi-segment original into new_m's payload */
  uint32_t copied = 0;
  struct rte_mbuf* seg = m;
  uint8_t* dst = (uint8_t*)rte_pktmbuf_mtod_offset(new_m, void*, hdr_len);
  while (seg && copied < payload_len) {
    uint32_t seglen = seg->data_len;
    if (seglen > payload_len - copied)
      seglen = payload_len - copied;
    rte_memcpy(dst + copied, rte_pktmbuf_mtod(seg, void*), seglen);
    copied += seglen;
    seg = seg->next;
  }
  if (copied != payload_len) {
    /* Should not happen, but be defensive */
    printf("Warning: copied %u != payload_len %u\n", copied, payload_len);
    rte_pktmbuf_free(new_m);
    return NULL;
  }

  /* Fill Ethernet header at the start of new_m */
  ...
  /* Fill IPv4 header immediately after Ethernet */
  struct rte_ipv4_hdr* ip_hdr = (struct rte_ipv4_hdr*)(eth_hdr + 1);
  memset(ip_hdr, 0, sizeof(*ip_hdr));
  ip_hdr->version_ihl = RTE_IPV4_VHL_DEF;
  ip_hdr->type_of_service = 0;
  /* IP total length = IP header + original packet length (we encapsulate entire
   * original) */
  uint16_t ip_total = (uint16_t)(sizeof(struct rte_ipv4_hdr) + payload_len);
  ip_hdr->total_length = rte_cpu_to_be_16(ip_total);
  ip_hdr->packet_id = rte_cpu_to_be_16(0);
  ip_hdr->fragment_offset = rte_cpu_to_be_16(0);
  ip_hdr->time_to_live = 64;
  ip_hdr->next_proto_id = IPPROTO_IPIP; /* IP-in-IP; change if needed */
...
/* Your encapsulation loop — unchanged except small style/defensive prints */
...
static int32_t add_cdev_mapping(struct rte_cryptodev_info* dev_info,
  ...
  for (i = dev_info->capabilities; i->op != RTE_CRYPTO_OP_TYPE_UNDEFINED; i++) {
    ...
    if (i->sym.xform_type == RTE_CRYPTO_SYM_XFORM_AEAD) {
      ret |=
          add_mapping(map, str, cdev_id, qp, params, ipsec_ctx, NULL, NULL, i);
      continue;
    }
...
static uint16_t cryptodevs_init(uint16_t req_queue_num) {
  ...
  const uint64_t mseg_flag =
      multi_seg_required() ? RTE_CRYPTODEV_FF_IN_PLACE_SGL : 0;
  ...
  for (cdev_id = 0; cdev_id < rte_cryptodev_count(); cdev_id++) {
    ...
    if ((mseg_flag & cdev_info.feature_flags) != mseg_flag)
      rte_exit(EXIT_FAILURE, "Device %hd does not support \'%s\' feature\n",
               cdev_id, rte_cryptodev_get_feature_name(mseg_flag));
...
static void port_init(uint16_t portid,
  ...
  ret =
      rte_eth_dev_configure(portid, nb_rx_queue, nb_tx_queue, &local_port_conf);
  if (ret < 0)
    rte_exit(EXIT_FAILURE,
  ...
  for (lcore_id = 0; lcore_id < RTE_MAX_LCORE; lcore_id++) {
    ...
    for (queue = 0; queue < qconf->nb_rx_queue; ++queue) {
      ...
      ret = rte_eth_rx_queue_setup(portid, rx_queueid, nb_rxd, socket_id,
...
static void session_pool_init(struct socket_ctx* ctx,
  ...
  sess_mp = rte_cryptodev_sym_session_pool_create(
      mp_name, nb_sess, sess_sz, CDEV_MP_CACHE_SZ, 0, socket_id);
...
static void session_priv_pool_init(struct socket_ctx* ctx,
  ...
  nb_sess = (get_nb_crypto_sessions() + CDEV_MP_CACHE_SZ * rte_lcore_count());
  nb_sess = RTE_MAX(nb_sess, CDEV_MP_CACHE_SZ * CDEV_MP_CACHE_MULTIPLIER);
  sess_mp = rte_mempool_create(mp_name, nb_sess, sess_sz, CDEV_MP_CACHE_SZ, 0,
...
static void pool_init(struct socket_ctx* ctx,
  ...
  ctx->mbuf_pool =
      rte_pktmbuf_pool_create(s, nb_mbuf, MEMPOOL_CACHE_SIZE,
                              ipsec_metadata_size(), frame_buf_size, socket_id);
  ...
  if (ms != 0) {
    ...
    ctx->mbuf_pool_indir = rte_pktmbuf_pool_create(
        s, nb_mbuf, MEMPOOL_CACHE_SIZE, 0, 0, socket_id);
  }
  ...
  for (i = 0; i != nb_pkts; i++) {
    ...
    if (eth->ether_type == rte_cpu_to_be_16(RTE_ETHER_TYPE_IPV4)) {
      ...
      if (rte_ipv4_frag_pkt_is_fragmented(iph)) {
        ...
        mb = rte_ipv4_frag_reassemble_packet(lc->frag.tbl, &lc->frag.dr, mb, tm,
                                             iph);
      ...
      if (fh != NULL) {
        ...
        tm = (tm != 0) ? tm : rte_rdtsc();
        mb = rte_ipv6_frag_reassemble_packet(lc->frag.tbl, &lc->frag.dr, mb, tm,
                                             iph, fh);
        if (mb != NULL)
...
static void inline_sessions_free(struct sa_ctx* sa_ctx) {
  ...
  for (i = 0; i < sa_ctx->nb_sa; i++) {
    ...
    ret = rte_security_session_destroy(rte_eth_dev_get_sec_ctx(sa->portid),
                                       ips->security.ses);
    if (ret)
      RTE_LOG(ERR, IPSEC,
...
int32_t main(int32_t argc, char** argv) {
  ...
  if (nb_bufs_in_pool == 0) {
    ...
    nb_bufs_in_pool =
        calculate_nb_mbufs(nb_ports, nb_crypto_qp, nb_rxq, nb_txq);
  }
  ...
  RTE_ETH_FOREACH_DEV(portid) {
    ...
    rte_eth_dev_callback_register(portid, RTE_ETH_EVENT_IPSEC,
                                  inline_ipsec_event_callback, NULL);
  }
